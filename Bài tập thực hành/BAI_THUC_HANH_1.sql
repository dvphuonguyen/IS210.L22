create user bai_thuc_hanh_1 identified by bai_thuc_hanh_1 default tablespace users;
grant create synonym, create any table to bai_thuc_hanh_1;
grant connect, resource to bai_thuc_hanh_1;
grant oem_monitor to bai_thuc_hanh_1;
grant dba to bai_thuc_hanh_1;
grant create view to bai_thuc_hanh_1;

connect bai_thuc_hanh_1;
...
------------------------5------------------------
select concat(concat(e.last_name,' '),e.first_name) "Employees", e.dept_id
from s_emp e 
where dept_id in (10, 50)
order by e.last_name;

-- hoac --
select (e.last_name||' '||e.first_name) "Employees", e.dept_id
from s_emp e 
where dept_id in (10, 50)
order by e.last_name;
...
------------------------7------------------------
/*select id, last_name, to_char(start_date, 'dd/mm/yyyy') 
from s_emp
where to_char(start_date,'dd/mm/yyyy') between '14/05/1990' and '26/05/1991'; */

select id, last_name, to_char(start_date, 'dd/mm/yyyy') 
from s_emp
where start_date between '14-MAY-90' and '26-MAY-91'; 
...
where to_char(start_date, 'yyyy') = 1991;
 ...
 ------------------------11------------------------
 select id, last_name, salary*1.15 "New Salary"
 from s_emp;
 
 
 
 ------------------------12------------------------
 select last_name, to_char(start_date, 'fmDdspth "of" month yyyy') "Ngay Tuyen Dung", to_char(next_day(add_months(start_date,6),'Monday'), 'fmDdspth "of" month yyyy') "Ngay Xem Xet Tang Luong"
 from s_emp;
...
------------------------14------------------------
select round(abs(months_between(sysdate, start_date)))
from s_emp
order by abs(months_between(sysdate, start_date));
...
------------------------17------------------------
select sp.name, sp.id, si.quantity "ORDERED"
from s_item si join s_product sp on si.product_id = sp.id
where ord_id = 101;
...
------------------------18------------------------
select sc.id, so.id
from s_customer sc left outer join s_ord so on sc.id = so.customer_id
order by sc.id;



------------------------19------------------------
select so.customer_id, si.product_id, quantity
from s_item si join s_ord so on si.ord_id = so.id
...
------------------------27------------------------
select sr.id, sr.name, sd.id
from s_region sr join s_dept sd on sr.id = sd.region_id



------------------------28------------------------
select sc.id, count(so.id) "So Luong Don Dat Hang"
from s_customer sc join s_ord so on so.customer_id = sc.id
group by sc.id
...
from s_customer sc join s_ord so on so.customer_id = sc.id
group by sc.id
having count(so.id) >= all ( select count(*)
                            from s_ord
...
------------------------30------------------------
select sc.id
from s_customer sc join s_ord so on sc.id = so.customer_id
group by sc.id
...
where dept_id = (select dept_id
                from s_emp
                where first_name = 'Lan' or last_name = 'Lan'
                )
...
------------------------33------------------------
select id, last_name ||' '|| first_name "Ho va ten"
from s_emp
where first_name like '%L%'and salary > (select avg(salary)
                                        from s_emp
                                        )
